...
import { Mail, Lock, EyeOff, Eye, Sparkles, Lightbulb, Loader2 } from "lucide-react";
...
export default function AuthPage() {
  ...
  const { loginWithEmail, registerUser, isLoading, error, clearError } = useAuthStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email || !password) {
      setLocalError("Silakan isi email dan password.");
      return;
    }
    ...
    try {
      if (!isLogin) {
        // Register Mode
        if (!name) {
          setLocalError("Silakan isi nama lengkap.");
          return;
        }
      ...
      setLocalError(err.message || "Terjadi kesalahan saat autentikasi.");
    }
  };
        ...
        {!isForgotPassword ? (
                  ...
                  <form className="space-y-4" onSubmit={handleSubmit}>
                    ...
                    {!isLogin ? (
                      <div className="space-y-4">
                        ...
                        <Button type="submit" disabled={isLoading} className="mt-2 h-12 w-full rounded-xl bg-[#6B4EFF] text-[15px] font-bold text-white shadow-md shadow-[#6B4EFF]/20 transition-all hover:bg-indigo-700">
                      ...
                      <div className="space-y-4">
                        ...
                        <Button type="submit" disabled={isLoading} className="mt-2 h-12 w-full rounded-xl bg-indigo-600 text-[15px] font-bold text-white shadow-md shadow-indigo-600/20 transition-all hover:bg-indigo-700">
          ...
          >
            ...
            <form className="mt-8 w-full" onSubmit={(e) => e.preventDefault()}>
              ...
              <Button type="submit" className="mt-6 h-[50px] w-full rounded-xl bg-[#6B4EFF] text-[15px] font-bold text-white transition-all hover:bg-indigo-700">
                Kirim Link Reset
              </Button>
            </form>
